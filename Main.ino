...
TFT_eSprite Icon15 = TFT_eSprite(&tft);
    ...
    if (SerialBT.available()) {
      ...
      if (param_start != -1 && param_end != -1) {
        ...
        String MEAS = "TV" + String(BMS.sum_voltage * 0.1) +  "/TC" + String((BMS.current - 30000) * 0.1) + "/SOC" +  String(BMS.SOC * 0.1) + "/TEMP" +  String(BMS.max_cell_temp - 40) +
                      ...
                      "/RC" + String(BMS.rem_cap) + "/BL" + String(balancerState) + "/SF"+ String(Safety) + "/#";
...
void BMS_COMM_CODE( void * pvParameters ) {
  ...
  for (;;) {
    ...
    if ((currentMillisBMS - previousMillisBMS) >= intervalBMS && (!HCC_HCD) && (!SLP_CAL) && (!HVC_LVC) && (!HVC_LVC_EQU) && (!HTC_HTD) && (!RC_RV)
        && (!BV_BD) && (!SOC_CAL) && (!SUM_CAL) && (!SOC_ALARM) && (!CHARGE_SET) && (!DISCHARGE_SET) && (!BALANCE_SET) && (!SafetyStartUp) && (!CAL_EQUVOLT) && (!CAL_EQUSUM) && (!CAL_EQUSLEEP) && (!CAL_EQUCURRENT) && (!CVD_CAL) && (!SafetyTurnOnApproved) && (!SafetyTurnOffApproved)) {
    ...
    else  //Parameter Set
    {
      ...
      if (SafetyTurnOnApproved || SafetyStartUp) {
    ...
    if ((WiFi.status() != WL_CONNECTED) && (currentMillis - previousMillis >= interval)) {
      ...
      if (udpcomm.listen(2001)) {
        udpcomm.onPacket([](AsyncUDPPacket packet) {
          ...
          //SET time OFFSET////////////////////////////////
          int offsetparam_start = UDPRequest.indexOf("SETOFFSET");
          int offsetparam_end = UDPRequest.indexOf("#");
          if (offsetparam_start != -1 && offsetparam_end != -1) {
            UDPprocessor = UDPRequest.substring(offsetparam_start + 9, offsetparam_end);
            Serial.println(UDPprocessor);
            EEPROMbusy = true;
            preferences.begin("my-app", false);
            preferences.putInt("timeoffset", UDPprocessor.toInt());
            preferences.end();
            EEPROMbusy = false;
          ...
          //SET CAP BMS////////////////////////////////
          int capparam_start = UDPRequest.indexOf("SETCAP");
          int capparam_end = UDPRequest.indexOf("#");
          if (capparam_start != -1 && capparam_end != -1) {
            UDPprocessor = UDPRequest.substring(capparam_start + 6, capparam_end);
          ...
          //SET SOC BMS////////////////////////////////
          int socparam_start = UDPRequest.indexOf("SETSOC");
          int socparam_end = UDPRequest.indexOf("#");
          if (socparam_start != -1 && socparam_end != -1) {
            UDPprocessor = UDPRequest.substring(socparam_start + 6, socparam_end);
           ...
           //TOGGLE SAFETY////////////////////////////////
          int safetyparam_start = UDPRequest.indexOf("TOGGLE_SAFETY");
          int safetyparam_end = UDPRequest.indexOf("#");
          if (safetyparam_start != -1 && safetyparam_end != -1) {
...
//SafetyTurnOnApproved
          ...
          int param_end = 0;
          int SLP;
          param_start = UDPRequest.indexOf("SETSLP");
          param_end = UDPRequest.indexOf("#");

          if (param_start != -1 && param_end != -1) {
            UDPprocessor = UDPRequest.substring(param_start + 6, param_end);
          ...
          //SET RTC BMS////////////////////////////////
          int RTC_INDEX_START = UDPRequest.indexOf("SETRTC");
          int RTC_INDEX_END = UDPRequest.indexOf("#");
          if (RTC_INDEX_START != -1 && RTC_INDEX_END != -1) {
            UDPprocessor = UDPRequest.substring(RTC_INDEX_START + 6, RTC_INDEX_END);
          ...
          //GET DRYCONTACTS BMS////////////////////////////////
          int GDRY_INDEX_START = UDPRequest.indexOf("GET_DRY");
          int GDRY_INDEX_END = UDPRequest.indexOf("#");
          if (GDRY_INDEX_START != -1 && GDRY_INDEX_END != -1) {
...
void TOUCH_SCREEN_CODE( void * pvParameters ) {
  ...
  for (;;) {
    if (ts.touched()) {
      ...
      if (PageNumber == 1) {
        //sAFETY
        if (!action_done && y >= 275 && y <= 320 && x >= 298 && x <= 330 && !numpadEnable ) {
          ...
          if (Safety) {
            SafetyTurnOff = true;
            SafetyTurnOn = false;
            Serial.println("Safety is requested to turn off");
          }
          else {
            SafetyTurnOff = false;
            SafetyTurnOn = true;
            Serial.println("Safety is requested to turn on");
          }
      ...
      if (PageNumber == 1 && SafetyTurnOff) {
        //Yes
        if (!action_done && y >= 205 && y <= 255 && x >= 90 && x <= 230 && !numpadEnable ) {
        ...
        if (!action_done && y >= 205 && y <= 255 && x >= 245 && x <= 355 && !numpadEnable ) {
...
void drawDashboard(int SOC, float MaxVolt, float MinVolt, float MaxDiff, float TerminalVolt, float TerminalCurr, float TerminalTemp) {
  ...
  if ((MaxVolt >= MinVolt && MaxVolt > 2.2 && MinVolt > 2.2) || TerminalTemp < 200 ) {
    ...
    Icon12.pushToSprite(&PageFrame, 160, 195, TFT_BLACK);
    PageFrame.drawString("Char.", 160, 180, 2);
    ...
    Icon13.pushToSprite(&PageFrame, 210, 195, TFT_BLACK);
    PageFrame.drawString("Dsch.", 210, 180, 2);
    ...
    Icon13.pushToSprite(&PageFrame, 260, 195, TFT_BLACK);
    PageFrame.drawString("Equ.", 260, 180, 2);
    ...
    PageFrame.setTextColor(TFT_WHITE, TFT_BLACK);
    Icon14.createSprite(24, 24);
    Icon14.setSwapBytes(true);


    if (Safety) {
      Icon14.pushImage(0, 0, 16, 16, locked);
    }
    else {
      Icon14.pushImage(0, 0, 16, 16, unlocked);
    }


    Icon14.pushToSprite(&PageFrame, 300, 197, TFT_BLACK);
    PageFrame.drawString("Safe", 298, 180, 2);
...
void drawSafetyNotification(TFT_eSprite & tft, bool Load) {
  ...
  PageFrame.fillRoundRect(60, 20, 360, 185, 5, TFT_BACKGROUND); //8
  PageFrame.setTextDatum(MC_DATUM);

  if (SafetyTurnOn) {

    if (!Load) {
      PageFrame.drawString("Are you sure ", 240, 50);
      PageFrame.drawString("to turn on Safety", 240, 80);


      PageFrame.fillRoundRect(90, 110, 140, 50, 5, TOLGA_RED); //8
      PageFrame.drawString("YES", 160, 130);
      PageFrame.fillRoundRect(245, 110, 140, 50, 5, TOLGA_RED); //8
      PageFrame.drawString("CANCEL", 320, 130);

    }
    else {
      PageFrame.drawString("There is load," , 240, 50);
      PageFrame.drawString("please ensure", 240, 80);
      PageFrame.drawString("load is disconnected", 240, 110);


      PageFrame.fillRoundRect(200, 135, 100, 50, 5, TOLGA_RED); //8
      PageFrame.drawString("OK", 250, 155);
  ...
  if (SafetyTurnOff) {
    PageFrame.drawString("Are you sure", 240, 50);
    PageFrame.drawString("to turn off Safety", 240, 80);
    PageFrame.fillRoundRect(90, 110, 140, 50, 5, TOLGA_RED); //8
    PageFrame.drawString("YES", 160, 130);
    PageFrame.fillRoundRect(245, 110, 140, 50, 5, TOLGA_RED); //8
    PageFrame.drawString("CANCEL", 320, 130);
...
void drawSafety() {
  ...
  drawSafetyNotification(NotificationFrame, abs((BMS.current - 30000) * 0.1) > 2 && (BMS.current - 30000) * 0.1 != -3000);
